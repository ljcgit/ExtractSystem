...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <div class="box-body">
          <form action="<%=basePath %>/InExcelServlet" method="post" enctype="multipart/form-data">
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <c:forEach items="${requestScope.staffs}" var="staff">  
				     ...
				     <td>${staff.phone}</td>  
				     <td>
					     <c:set var="status" scope="request" value="${staff.status}"/>
			   			<c:choose>
			   				<c:when test="${staff.status==0}">  
								<span class="label label-success"> 可参加 </span>  
							</c:when>
							<c:when test="${staff.status==1}">  
								<span class="label label-warning"> 已请假 </span>  
							</c:when>
							<c:when test="${staff.status==2}">  
								<span class="label label-primary"> 已参加 </span>  
							</c:when>
						</c:choose>  
				 	</td>
				 	<td>
				 		<button type="button" class="btn btn-warning" onclick="window.location.href='<%= basePath %>/OperatorServlet?action=leave&id=${staff.id } '">请假</button>
						<button type="button" class="btn btn-danger" onclick="window.location.href='<%= basePath %>/OperatorServlet?action=delete&id=${staff.id}'">删除</button>
				 	</td>
		  	 </tr>  
		</c:forEach> 
   				</tbody>
